...
if [ -z "$GOOGLE_CLIENT_ID" ]; then
    bashio::log.fatal "client_id is not set in add-on config"; exit 1
fi
if [ -z "$GOOGLE_CLIENT_SECRET" ]; then
    bashio::log.fatal "client_secret is not set in add-on config"; exit 1
fi
if [ -z "$GOOGLE_REFRESH_TOKEN" ]; then
    bashio::log.fatal "refresh_token is not set in add-on config"; exit 1
fi
